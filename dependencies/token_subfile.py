...
def convert_xls_to_xlsx_format(df):
    df['Instant debit amount'] = df['Instant debit amount'].astype(str).fillna('').replace('nan', '')
    columns_to_convert = ['Card_Number', 'Payment gateway response code']
    df[columns_to_convert] = df[columns_to_convert].astype(str)
    return df

def create_token_table_format():
  tokenfile_format = {
...
def copy_data_to_token_table(new_df, df):
...
def process_egfile(folder_path, file_name):
    ...
    df = convert_xls_to_xlsx_format(df)
...
def process_new_tokenfile(folder_path, file_name):
    ...
    new_df = create_token_table_format()
    new_df = copy_data_to_token_table(new_df, df)
    new_df = mark_card_type(new_df)
...
def status_prompt(new_file_name):
   ...
   logging.info(f'{new_file_name} has been successfully created!')
